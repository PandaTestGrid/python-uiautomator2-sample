...
class TestDeviceInfo(BaseTest):
    ...
    def test_device_basic_info(self):
        ...
        # 验证必要字段（uiautomator2 返回的字段）
        assert "displayWidth" in info or "displaySizeDpX" in info, "设备信息缺少显示信息"
        assert "currentPackageName" in info, "设备信息缺少当前包名"

        self.logger.info(f"设备信息: {info}")
        self.logger.info(f"显示宽度: {info.get('displayWidth')}")
        self.logger.info(f"显示高度: {info.get('displayHeight')}")
        self.logger.info(f"当前包名: {info.get('currentPackageName')}")
    ...
    def test_device_screen_on(self):
        ...
        测试检查屏幕是否点亮
        """
        is_screen_on = self.device.info.get("screenOn", False)
        self.logger.info(f"屏幕状态: {'点亮' if is_screen_on else '熄灭'}")
    
    @pytest.mark.android
    def test_device_battery_info(self):
        """
        测试获取电池信息
        """
        try:
            # 通过 shell 命令获取电池信息
            battery_level = self.device.shell("dumpsys battery | grep level").output.strip()
            if battery_level:
    ...
    @pytest.mark.android
    def test_device_memory_info(self):
        """
        测试获取内存信息
        """
        try:
            # 获取内存信息
            mem_info = self.device.shell("cat /proc/meminfo | head -3").output
            self.logger.info(f"内存信息:\n{mem_info}")
        except Exception as e:
            self.logger.warning(f"获取内存信息失败: {e}")
    
    @pytest.mark.android
    def test_device_cpu_info(self):
        """
        测试获取 CPU 信息
        """
        try:
            # 获取 CPU 信息
            cpu_info = self.device.shell("cat /proc/cpuinfo | head -10").output
